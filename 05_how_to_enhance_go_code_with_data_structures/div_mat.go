...
func randomDivMat(min, max int) float64 {
...
func adjoint(A [][]float64) ([][]float64, error) {
	N := len(A)
	adj := createDivMatrix(N, N)
	if N == 1 {
		adj[0][0] = 1
		return adj, nil
	}
	sign := 1
	var temp = createDivMatrix(N, N)
	for i := 0; i < N; i++ {
		for j := 0; j < N; j++ {
...
func inverseMatrix(A [][]float64) ([][]float64, error) {
	N := len(A)
	var inverse = createDivMatrix(N, N)
	det := determinant(A, N)
	if det == 0 {
...
func createDivMatrix(row, col int) [][]float64 {
	r := make([][]float64, row)
	for i := 0; i < row; i++ {
		for j := 0; j < col; j++ {
			r[i] = append(r[i], randomDivMat(-5, i*j))
		}
	}
	return r
}
...
func main() {
	...
	m1 := createDivMatrix(row, col)
	m2 := createDivMatrix(row, col)
	fmt.Println("m1:", m1)
	fmt.Println("m2:", m2)
